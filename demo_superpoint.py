...
class PointTracker(object):
    ...
    def draw_tracks(self, out, tracks):
        ...
        for track in tracks:   #tracks為匹配成功的特徵點索引Array
    ...
    def return_keyPoint(self,tracks):
        # Store the number of points per camera.
        pts_mem = self.all_pts
        N = len(pts_mem)  # Number of cameras/images.
        # Get offset ids needed to reference into pts_mem.
        ...
        # Iterate through each track and draw it.
        for track in tracks:  # tracks為匹配成功的特徵點Array
            num += 1
            for i in range(N - 1):
                if track[i + 2] == -1 or track[i + 3] == -1:
                    continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', type=str, default=ROOT / 'assets/icl_snippet',
    ...
    while True:
        ...
        # 回傳特徵點 src_pts,dst_pts
        src_pts,dst_pts = tracker.return_keyPoint(tracks)
        print(src_pts)
        print(dst_pts)
        if opt.show_extra:
